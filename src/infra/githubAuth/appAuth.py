...
class GithubAppAuth(GithubAuth):
    def __init__(self, config: GithubAppConfig, client: HttpClient | None = None, clock: Clock | None = None) -> None:
        self.cfg = config
        self._client = RequestsHttpClient() if client is None else client
        self._clock = clock if clock is not None else SystemClock()
        self._jwt_cache: Tuple[str, int] | None = None # Cache for JWT tokens with their expiration times
    ...
    def headers(self, ctx: GithubAuthContext) -> Mapping[str, str]:
        if ctx.installation_id is None:
            raise TransportError("Installation ID is required for GitHub App authentication")
    ...
    def _get_claims(self) -> dict[str, Any]:
        ...
        return {
            "iat": now - 30, # issued 30 seconds in the past to allow deviations in time
            "exp": now + 9 * 60, # 9 minutes expiration (max 10 minutes allowed by GitHub)
            "iss": self.cfg.app_id,
        }
    ...
    def _fetch_installation_token(self, installation_id: int) -> GithubTokenResponse:
        ...
        if not response.ok:
            raise TransportError(f"Failed to obtain installation token: {response.status_code} {response.text}")
